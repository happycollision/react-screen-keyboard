	...
	function Keyboard() {
		...
		return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Keyboard.__proto__ || Object.getPrototypeOf(Keyboard)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
			...
			if (_this.props.onChange) {
				...
				_this.props.inputNode.value = nextValue;
			}

			if (_this.props.onClick) {
				_this.props.onClick(nextValue);
			}
		}, _this.setCursor = function (nextPos) {
		...
		}, _this.notifyNewState = function (value, cursorPosition) {
			if (_this.props.onUpdate) _this.props.onUpdate({ value: value, cursorPosition: cursorPosition });
		}, _this.makeChangesToInput = function (nextValue, nextPos) {
			_this.changeValue(nextValue);
			_this.setCursor(nextPos).then(function () {
				return _this.notifyNewState(nextValue, nextPos);
			});
		}, _this.setKeyboardCase = function () {
			_this.setState({ uppercase: _this.isUppercase() });
		}, _this.getNextStateForLetterClick = function (key) {
			var inputNode = _this.props.inputNode;
		...
		}, _this.getNextStateForBackspaceClick = function () {
			var inputNode = _this.props.inputNode;
			var value = inputNode.value,
			    selectionStart = inputNode.selectionStart,
			    selectionEnd = inputNode.selectionEnd;
			...
			var next = _this.getNextStateForLetterClick(key);
			_this.makeChangesToInput(next.value, next.cursorPosition);
			_this.setKeyboardCase();
		}, _this.handleBackspaceClick = function () {
			var next = _this.getNextStateForBackspaceClick();
			_this.makeChangesToInput(next.value, next.cursorPosition);
			_this.setKeyboardCase();
